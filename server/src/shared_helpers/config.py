...
def get_organization_config(org_id):
    ...
    try:
        with open(os.path.join(CONFIGS_PARENT_DIR, "organizations", org_id + ".yaml")) as f:
...
def get_path_to_oauth_secrets():
    ...
    if not os.path.isfile(production_path):
        if env.current_env_is_local():
            return os.path.join(
                os.path.dirname(__file__), "../local/client_secrets_local_only.json"
            )
        else:
...
def get_default_namespace(org_id):
    ...
    try:
        if (
            session.get("org_default_ns_exp")
            and session.get("org_default_ns_exp").replace(tzinfo=None) > datetime.datetime.utcnow()
        ):
    ...
    if not default_namespace:
        default_namespace = get_organization_config(org_id).get(
            "default_namespace", get_config_by_key_path(["default_namespace"]) or "go"
        )

        try:
            session["org_default_ns_exp"] = datetime.datetime.utcnow() + datetime.timedelta(
                days=CACHE_DURATION_IN_DAYS
            )
