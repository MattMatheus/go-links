...
def _matches_pattern(keywords_punctuation_sensitive, provided_shortpath, candidate_match):
    ...
    for i in range(len(provided_shortpath_components)):
        ...
        elif candidate_match_components[i] != get_canonical_keyword(
            keywords_punctuation_sensitive, provided_shortpath_components[i]
        ):
            return None
...
def derive_pattern_match(organization, keywords_punctuation_sensitive, namespace, shortpath):
    ...
    prefix_matches = models.ShortLink.get_by_prefix(
        organization, namespace, shortpath.split("/")[0]
    )
    ...
    for prefix_match in prefix_matches:
        ...
        matching_formatting_args = _matches_pattern(
            keywords_punctuation_sensitive, shortpath, prefix_match.shortpath
        )
    ...
    return matching_shortlink, matching_shortlink.destination_url.replace("%", "%%").replace(
        "%%s", "%s"
    ) % tuple(matching_formatting_args)
...
def get_shortlink(
    organization, keywords_punctuation_sensitive, alternative_resolution_mode, namespace, keyword
):
    ...
    matching_programmatic_link, destination = derive_pattern_match(
        organization, keywords_punctuation_sensitive, namespace, keyword
    )
    ...
    if alternative_resolution_mode:
        ...
        if len(keyword_parts) > 1:
            perfect_prefix_match = models.ShortLink.get_by_full_path(
                organization, namespace, keyword_parts[0]
            )

            if perfect_prefix_match:
                return perfect_prefix_match, perfect_prefix_match.destination_url + "/".join(
                    non_canonical_keyword_parts[1:]
                )
        else:
            ...
            if len(prefix_matches) > 1:
                logging.error(
                    "Multiple prefix matches for keyword %s in org %s, which is using "
                    "alternative resolution mode",
                    organization,
                    keyword,
                )
...
def find_conflicting_link(organization, namespace, shortpath):
    ...
    links_with_prefix = models.ShortLink.get_by_prefix(organization, namespace, shortpath_prefix)
...
def update_short_link(acting_user, link_object):
    return upsert_short_link(
        acting_user,
        link_object.organization,
        link_object.owner,
        link_object.namespace,
        link_object.shortpath,
        link_object.destination_url,
        link_object,
    )
...
def check_namespaces(organization, namespace, shortpath):
    ...
    if namespace != default_namespace and namespace not in org_namespaces:
        raise LinkCreationException(
            '"%s" is not a valid link prefix for your organization' % (namespace)
        )

    # If an organization has a namespace of, for example, "eng", go/eng can be created,
    # but go/eng/%s would conflict with eng/something, as {base}/eng/something is how
    # users without an extension installed can still easily
    # access links in the "eng" namespace.
    if namespace == default_namespace:
        shortpath_parts = shortpath.split("/")
        if len(shortpath_parts) > 1 and shortpath_parts[0] in org_namespaces:
            raise LinkCreationException(
                '"%s" is a reserved prefix for your organization' % (shortpath_parts[0])
            )


SIMPLE_VALIDATION_MODE = "simple"
EXPANDED_VALIDATION_MODE = "expanded"
PATH_RESTRICTIONS_ERROR_SIMPLE = (
    'Keywords can include only letters, numbers, hyphens, "/", and "%s" placeholders'
)
PATH_RESTRICTIONS_ERROR_EXPANDED = (
    "Provided keyword is invalid"  # TODO: Include invalid char(s) in error
)
KEYWORDS_PUNCTUATION_SENSITIVE_DEFAULT = True
ALTERNATIVE_KEYWORD_RESOLUTION_MODE = "alternative"
DEFAULT_VALIDATION_REGEX = r"[^0-9a-zA-Z\-\/%]"


def get_keyword_validation_regex(org_settings):
    return (
        get_from_key_path(org_settings or {}, ["keywords", "validation_regex"])
        or DEFAULT_VALIDATION_REGEX
    )
...
"""Returns whether or not the provided destination is a valid URL using a bare hostname,
like `http://go/directory`. This function is used to work around a limitation of the
`validators` package where it doesn't recognize URLs using bare hostnames as valid.
"""
...
"""Returns whether or not the provided destination is a valid URL using double hyphen,
like `https://double--hyphen.example.com/some/path`. This function is used to work around
a limitation of the `validators` package where it doesn't recognize URLs with double
hypens valid
"""


def _is_valid_idn_destination(destination):
    clean_url = re.sub(r"\-\-+", "-", destination)
...
def _validate_destination(destination):
    ...
    if (
        not validators.url(destination_base)
        and not _is_valid_bare_hostname_destination(destination_base)
        and not _is_valid_idn_destination(destination_base)
    ):
...
def are_keywords_punctuation_sensitive(org_config):
    keywords_punctuation_sensitive = config.get_from_key_path(
        org_config, ["keywords", "punctuation_sensitive"]
    )
...
def is_using_alternative_keyword_resolution(org_config):
    return ALTERNATIVE_KEYWORD_RESOLUTION_MODE == config.get_from_key_path(
        org_config, ["keywords", "resolution_mode"]
    )
...
def upsert_short_link(
    ...
    destination,
    updated_link_object,
    validation_mode=EXPANDED_VALIDATION_MODE,
    unlisted=False,
):
    ...
    if (
        org_config.get("read_only_mode")
        and not updated_link_object
        and validation_mode != EXPANDED_VALIDATION_MODE
    ):
    ...
    if not (
        organization == acting_user.organization and owner == acting_user.email
    ...
    if len(shortpath_parts) > 1:
        if alternative_resolution_mode:
            if shortpath_parts[1] != "%s":
                raise LinkCreationException(
                    'Only "%s" placeholders are allowed after the first "/" in a keyword'
                )
    ...
    if not updated_link_object:
        ...
        if existing_link:
            error_message = "That go link already exists. %s/%s points to %s" % (
                namespace,
            ...
            if existing_link.shortpath != shortpath:
                error_message = "A conflicting go link already exists. %s/%s points to %s" % (
                    namespace,
        ...
        if "%s" in shortpath:
            ...
            if conflicting_link:
                raise LinkCreationException(
                    "A conflicting go link already exists. %s/%s points to %s"
                    % (namespace, conflicting_link.shortpath, conflicting_link.destination_url)
                )

    # Note: urlparse('128.90.0.1:8080/start').scheme returns '128.90.0.1'.
    destination_url_scheme = urlparse(destination).scheme
    if (
        not destination_url_scheme
