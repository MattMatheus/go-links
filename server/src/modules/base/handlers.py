...
# Description of changes: Style corrections and expand error handle
...
def login_via_test_token():
    ...
    if payload["user_email"].split("@")[1] not in get_config()["testing"]["domains"]:
        raise Exception(
            "Invalid test user %s, with test token: %s"
            % (payload["user_email"], request.args.get("test_token"))
        )
...
def _redirect():
    if session.get("redirect_to_after_oauth", "").startswith(
        authentication.get_host_for_request(request) + "/"
    ):
...
def opensearch():
    return Response(
        response=render_template("opensearch/manifest.xml"),
        status=200,
