...
# - Pruned reference to commercial functions
...
from shared_helpers.services import (
...
def get_allowed_authentication_methods(organization):
    try:
        return service_get("admin", f"/organizations/{quote(organization)}/settings").get(
...
def login(authentication_method, user_id=None, user_email=None, user_org=None):
    ...
    if user_id:
        ...
        user = get_or_create_user(
            user_email, user_org if user_org else get_organization_id_for_email(user_email)
        )

    allowed_authentication_methods = get_allowed_authentication_methods(user.organization)
    if (
        allowed_authentication_methods is not None
        and authentication_method not in allowed_authentication_methods
    ):
        logging.warning(
        ...
        if handle_unsupported_signin is not None:
            redirect_override = handle_unsupported_signin(
                user.organization, authentication_method, allowed_authentication_methods
            )
...
def get_user_email(oauth_credentials):
    ...
    if type(email_verified) is not bool:
        raise Exception(
            f"'email_verified' property is wrong type ({type(email_verified).__name__}). "
            " Should be bool."
        )
...
def get_host_for_request(request):
    ...
    return "%s://%s" % (
        "http" if host == "localhost" or host.startswith("localhost:") else "https",
        host,
    )
